...
def run_null_hypothesis_test():
    ...
    for i in range(n_trials):
        ...
        res_c = solver.solve(max_epochs=2000) # Silent usually, but my solver prints
    ...
    if chimera_wins > random_wins or (chimera_wins == random_wins and avg_c < avg_r):
        print("\nCONCLUSION: CAUTION - SIMULATION MODE.")
        print("In simulation, 'Physical Chaos' is just PRNG.")
        print("We expect performance to be similar to Random or slightly WORSE due to overhead.")
        print("Real Phase Transitions require the S9 Hardware (Non-Markovian Physics).")
    else:
        print("\nCONCLUSION: Null Hypothesis Holds (as expected for CPU simulation).")
